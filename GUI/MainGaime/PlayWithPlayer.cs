...
namespace GUI
{
    ...
    public partial class PlayWithPlayer : Form
    {
        ...
        private void MainGameGUI_Load(object sender, EventArgs e)
        {
            ...
            //random bot avatar
            Pbx_BotAvt.Image = (Image)Program.Dic_Bundles[StringManagement.KeyDatas.PlayerAvatar_Key2];
        ...
        private void Pnl_Choice_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (Turn_Flag && !isFinish() && !Flag_PlayClick && ((Panel)sender).Controls.Count > 0 && isPlayerClick)
            {
                ...
                Pnl_ChoiceTemp = (Panel)sender;  // Lưu lại ô được chọn
                idxChoiceCell = getIdxChoice(Pnl_ChoiceTemp);  // Lấy chỉ số ô được chọn
                showImage_MouseClick(Pnl_ChoiceTemp);  // Hiển thị ảnh khi click

                Pnl_TrooperBin = Pnl_PlayerBox;  // Chuyển quân về box của người chơi 1
                CellBLL_Choice = new CellBLL(idxChoiceCell, int.Parse(Lbl_CellTable[idxChoiceCell].Text));  // Lưu lại thông tin ô đã chọn
            }

            else if (!Turn_Flag && !isFinish())
            {
                // Nếu là lượt của người chơi 2, xử lý như người chơi 1
                Program.Dic_Sounds[SoundKind.CHOICE_SOUND].windowsMediaPlayer.controls.play();

                // Cập nhật các giá trị của người chơi 2
                ...
                Pnl_TrooperBin = Pnl_BotBox;  // Chuyển quân về box của người chơi 2
                CellBLL_Choice = new CellBLL(idxChoiceCell, int.Parse(Lbl_CellTable[idxChoiceCell].Text));  // Lưu lại thông tin ô đã chọn

                //// Tạo một popup hoặc một hình ảnh để người chơi chọn hướng di chuyển
                //ShowMoveDirectionOptions();  // Hiển thị các lựa chọn hướng di chuyển
            }
        }

        private void ShowMoveDirectionOptions()
        {
            // Tạo 2 nút chọn di chuyển trái và phải
            Button btnMoveLeft = new Button();
            btnMoveLeft.Text = "Move Left";
            btnMoveLeft.Click += (s, e) => MovePlayerDirection(false);  // Di chuyển trái

            Button btnMoveRight = new Button();
            btnMoveRight.Text = "Move Right";
            btnMoveRight.Click += (s, e) => MovePlayerDirection(true);  // Di chuyển phải

            // Tạo panel để chứa các nút này
            Panel moveOptionsPanel = new Panel();
            moveOptionsPanel.Controls.Add(btnMoveLeft);
            moveOptionsPanel.Controls.Add(btnMoveRight);

            // Hiển thị panel này trên giao diện
            this.Controls.Add(moveOptionsPanel);
        }

        // Hàm xử lý di chuyển sau khi người chơi chọn hướng
        private void MovePlayerDirection(bool check)
        {
            // Thực hiện di chuyển người chơi 2
            if (check)
            {
            ...
            // Cập nhật lại route
            RouteBll_Route.calRoute(CellBLL_Choice, check);
            RouteBll_Route.load();

            // Cập nhật lại lượt và trạng thái sau khi người chơi 2 chọn xong
            Turn_Flag = true;  // Chuyển lượt cho người chơi 1
            countDown = 10;    // Reset thời gian cho người chơi 1
            Lbl_Clock.Text = countDown.ToString();  // Hiển thị lại đồng hồ cho người chơi 1
            Lbl_Clock.ForeColor = Color.Black;  // Đổi màu chữ đồng hồ về đen
            Timer_Clock.Start();  // Bắt đầu đồng hồ cho người chơi 1

            // Xóa các nút di chuyển sau khi chọn
            this.Controls.RemoveAt(this.Controls.Count - 1);  // Xóa panel chứa các nút di chuyển
        }
        ...
        private void Pnl_ChoicePlayer2()
        {
            if (!isFinish())
            {
                Program.Dic_Sounds[SoundKind.CHOICE_SOUND].windowsMediaPlayer.controls.play();
                ...
                // Người chơi 2 chọn ô
                Pnl_ChoiceTemp = Pnl_CellTable[idxChoiceCell];  // idxChoiceCell là ô người chơi 2 chọn
                CellBLL_Choice = new CellBLL(idxChoiceCell, int.Parse(Lbl_CellTable[idxChoiceCell].Text));

                // Di chuyển quân cờ (giống như người chơi 1)
                if (dirBotMovement)  // Giả sử `dirBotMovement` là biến xác định hướng di chuyển, bạn có thể thay đổi cách thức di chuyển tùy theo yêu cầu
                    moveRight();
                else
                    moveLeft();

                // Tính toán lộ trình di chuyển cho player 2
                RouteBll_Route.calRoute(CellBLL_Choice, dirBotMovement);
                RouteBll_Route.load();

                // Cập nhật lại lượt chơi
                Turn_Flag = true;  // Đổi lượt trở lại cho người chơi 1
                countDown = 10;  // Đặt lại thời gian cho người chơi 1
                Lbl_Clock.Text = "10";  // Hiển thị lại thời gian cho người chơi 1
                Lbl_Clock.ForeColor = Color.Black;  // Đổi màu đồng hồ về màu đen

                // Bắt đầu lại đồng hồ cho người chơi 1
                Timer_Clock.Start();
            }
        }
        ...
        private int delayClock = 2;
        private bool showTurn = false;
        private void Timer_Clock_Tick(object sender, EventArgs e)
        {
            finishGame();  // Kiểm tra trò chơi đã kết thúc hay chưa

            if (Turn_Flag) // Người chơi 1
            {
                int currTime = Int32.Parse(Lbl_Clock.Text);

                // Cập nhật màu đồng hồ
                if (currTime <= 5)
                ...
                {
                    showTurn = true;
                    Tnf_TurnNotification.Lbl_Turn.Text = StringManagement.InitializePlayerTurn();
                    Tnf_TurnNotification.run();
                }

                // Xử lý khi người chơi 1 phân phối quân
                if (isSpread())
                {
                    Lbl_TrooperBin = Lbl_ScoreTroopePlayerBox;
                ...
                // Nếu người chơi 1 chưa chọn ô khi hết giờ, hệ thống sẽ chọn ngẫu nhiên
                ...
                if (currTime < 0)
                {
                    ...
                    // Chọn ô ngẫu nhiên từ 7 đến 11 nhưng chỉ chọn ô hợp lệ (không có quân)
                    ...
                    if (!isFinish())
                    {
                        ...
                        // Di chuyển quân (chọn ngẫu nhiên trái/phải)
            ...
            else // lượt của người chơi 2
            {
                if (showTurn)
                {
                    showTurn = false;
                    Tnf_TurnNotification.Lbl_Turn.Text = StringManagement.InitializePlayerTurn2(); // Hiển thị "Lượt của người chơi 2"
                    Tnf_TurnNotification.run();
                }

                // Ẩn đồng hồ người chơi 1
                ...
                // Xử lý khi người chơi 2 phân phối quân
                if (!Turn_Flag && isSpread()) // Nếu là lượt của người chơi 2 và người chơi 2 đang phân phối quân
                {
                    // Cập nhật bin quân cho người chơi 2
                    Lbl_TrooperBin = Lbl_ScoreTroopeBotBox;

                    // Phân phối quân cho người chơi 2
                    spreadTroopers(Pnl_BotBox);
                ...
                if (currTime < 0 && !Turn_Flag)  // Nếu là lượt của người chơi 2 và hết thời gian
                {
                    isPlayerClick = false;
                    Timer_Clock.Stop();

                    Random random = new Random();
                    // Chọn ô ngẫu nhiên từ 0 đến 5 (chỉ chọn ô hợp lệ)
                    while (Int32.Parse(Lbl_CellTable[(idxChoiceCell = random.Next(0, 6))].Text) == 0) { }

                    if (!isFinish())  // Kiểm tra xem trò chơi có kết thúc không
                    {
                        // Xóa các mũi tên khỏi các ô của người chơi 2
                        for (int i = 0; i <= 5; i++)
                        {
                            Pnl_CellTable[i].Controls.RemoveByKey("Pbx_LeftArrow");
                            Pnl_CellTable[i].Controls.RemoveByKey("Pbx_RightArrow");
                        ...
                        // Cập nhật thông tin cho người chơi 2
                        Pnl_ManBotBin = Pnl_BotMandarin2;
                        Pnl_ManPlayerBin = Pnl_PlayerMandarin2;
                        ...
                        // Di chuyển quân (ngẫu nhiên trái/phải)
                        if ((random.Next(0, 2) != 0))
                        {
                            moveRight();
                            RouteBll_Route.calRoute(CellBLL_Choice, true);
                ...
                if (delayClock > 0)
                {
                    delayClock = 2;    // Đặt lại thời gian delay
                    Lbl_Clock.Text = countDown.ToString();
                    if (isPlayerClick) // Nếu người chơi 2 đã chọn, chuyển lượt
                    {
                        Turn_Flag = true;  // Chuyển lượt cho người chơi 1
                        countDown = 10;    // Reset thời gian cho người chơi 1
                        Lbl_Clock.Text = countDown.ToString();
                        Timer_Clock.Start();  // Bắt đầu lại Timer cho người chơi 1
                    }
                    else
                    {
                        Timer_Clock.Start();
